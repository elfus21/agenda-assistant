...
function parseTextToEvent(text) {
    ...
    const title = lower.includes("excursión")
        ? "Excursión"
    ...
    const notes = [];
    if (lower.includes("no llevar mochila"))
        notes.push("No llevar mochila");
    if (lower.includes("llevar gorra"))
        notes.push("Llevar gorra");
    if (lower.includes("llevar agua"))
        notes.push("Llevar agua");
    if (lower.includes("camiseta blanca"))
        notes.push("Preparar camiseta blanca");
    const reminders = title === "Excursión" || title === "Dentista" || title === "Cumpleaños" || title === "Reunión"
        ? [
            {
                when: "day_before",
    ...
    const checklist = [];
    if (title === "Excursión")
        checklist.push("Revisar indicaciones");
